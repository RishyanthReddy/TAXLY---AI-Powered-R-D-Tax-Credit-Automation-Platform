...
This module provides comprehensive monitoring for all API connectors including:
...
The monitor tracks all API interactions across Clockify, Unified.to, You.com,
OpenRouter (GLM), and other external services.
"""
...
logger = get_tool_logger("api_monitor")
...
class APICallMetric:
    ...
    Attributes:
        api_name: Name of the API (Clockify, Unified.to, You.com, OpenRouter, etc.)
...
class APIMonitor:
    """
    Monitor for all API usage and performance.
    
    This class tracks all API calls across different services, response times,
    errors, and provides alerting capabilities for repeated failures.
    
    Features:
    - Per-API and per-endpoint call tracking
    ...
    def __init__(
        ...
        Example:
            >>> monitor = APIMonitor()
            >>> # Monitor will track API calls when record_api_call() is called
            >>> 
        ...
        # Statistics storage
        # Key: (api_name, endpoint) -> EndpointStatistics
        self.endpoint_stats: Dict[tuple, EndpointStatistics] = {}
    ...
    def record_api_call(
        ...
        This method should be called after each API call to track
        ...
        with self.lock:
            ...
            key = (api_name, endpoint)
            if key not in self.endpoint_stats:
                self.endpoint_stats[key] = EndpointStatistics(
                    api_name=api_name,
                    endpoint=endpoint
                )
            
            self.endpoint_stats[key].update(metric)
            
            # Check for alerts
            if self.enable_alerts:
                self._check_alerts(api_name, endpoint, metric)
    
    def _check_alerts(self, api_name: str, endpoint: str, metric: APICallMetric):
        """
        ...
        if stats.consecutive_failures >= self.failure_threshold:
            logger.error(
                f"[ALERT] {api_name} API: {endpoint} has {stats.consecutive_failures} "
                f"consecutive failures (threshold: {self.failure_threshold})\n"
                f"Last error: {metric.error_type} - {metric.error_message}\n"
                f"Recommendation: Check API key, network connectivity, and service status"
            )
        ...
        if stats.total_calls >= 10:
            ...
            if error_rate >= self.error_rate_threshold:
                logger.warning(
                    f"[ALERT] {api_name} API: {endpoint} has high error rate "
        ...
        if metric.success and metric.response_time >= self.slow_response_threshold:
            logger.warning(
                f"[ALERT] {api_name} API: {endpoint} slow response "
    ...
    def get_api_statistics(self, api_name: str) -> Dict[str, Any]:
        """
        Get statistics for a specific API (all endpoints).
        ...
        with self.lock:
            ...
            # Calculate overall statistics for this API
            total_calls = sum(s.total_calls for s in api_endpoints.values())
            total_successful = sum(s.successful_calls for s in api_endpoints.values())
            total_failed = sum(s.failed_calls for s in api_endpoints.values())
            total_cached = sum(s.cached_calls for s in api_endpoints.values())
            total_response_time = sum(s.total_response_time for s in api_endpoints.values())
            total_wait_time = sum(s.total_wait_time for s in api_endpoints.values())
            ...
            return {
                ...
                'endpoints': {
                    key[1]: stats.to_dict()
                    for key, stats in api_endpoints.items()
                }
            }
    
    def get_endpoint_statistics(self, api_name: str, endpoint: str) -> Optional[Dict[str, Any]]:
        """
        ...
        Example:
            >>> monitor = APIMonitor()
            >>> # ... make some API calls ...
            >>> stats = monitor.get_endpoint_statistics('Clockify', '/time-entries')
            >>> print(f"Total calls: {stats['total_calls']}")
        ...
        with self.lock:
            ...
            if key not in self.endpoint_stats:
                return None
            return self.endpoint_stats[key].to_dict()
    
    def get_all_statistics(self) -> Dict[str, Any]:
        """
        Get statistics for all APIs and endpoints.
        
        Returns:
            Dictionary with overall statistics and per-API statistics
        ...
        Example:
            ...
            ...     print(f"{api_name}: {api_stats['total_calls']} calls")
        """
        with self.lock:
            ...
            # Group by API
            apis = {}
            for (api_name, endpoint), stats in self.endpoint_stats.items():
                if api_name not in apis:
                    apis[api_name] = []
                apis[api_name].append(stats)
            
            # Calculate per-API statistics
            api_stats = {}
            for api_name, endpoint_list in apis.items():
                api_total_calls = sum(s.total_calls for s in endpoint_list)
                api_successful = sum(s.successful_calls for s in endpoint_list)
                api_failed = sum(s.failed_calls for s in endpoint_list)
                api_cached = sum(s.cached_calls for s in endpoint_list)
                api_response_time = sum(s.total_response_time for s in endpoint_list)
                api_wait_time = sum(s.total_wait_time for s in endpoint_list)
                
                api_stats[api_name] = {
                    'total_calls': api_total_calls,
                    'successful_calls': api_successful,
                    'failed_calls': api_failed,
                    'cached_calls': api_cached,
                    'error_rate': (api_failed / api_total_calls * 100.0) if api_total_calls > 0 else 0.0,
                    'cache_hit_rate': (api_cached / api_total_calls * 100.0) if api_total_calls > 0 else 0.0,
                    'average_response_time': (api_response_time / api_successful) if api_successful > 0 else 0.0,
                    'total_wait_time': api_wait_time,
                    'endpoints': {
                        s.endpoint: s.to_dict() for s in endpoint_list
                    }
                }
    ...
    def get_cost_estimate(self) -> Dict[str, Any]:
        ...
        Returns usage counts per API for cost estimation. Actual costs depend
        on your specific API plans and pricing.
        
        Returns:
            Dictionary with usage counts per API for cost estimation
        
        Example:
            >>> monitor = APIMonitor()
            >>> # ... make some API calls ...
            >>> cost_info = monitor.get_cost_estimate()
            >>> print(f"Clockify API calls: {cost_info['Clockify']['total_calls']}")
            >>> print(f"You.com API calls: {cost_info['You.com']['total_calls']}")
        """
        with self.lock:
            cost_info = {}
            ...
            # Calculate per-API usage
            for api_name, endpoint_list in apis.items():
                total_calls = sum(s.total_calls for s in endpoint_list)
                successful_calls = sum(s.successful_calls for s in endpoint_list)
                cached_calls = sum(s.cached_calls for s in endpoint_list)
                billable_calls = successful_calls - cached_calls
                ...
                cost_info[api_name] = {
                    ...
                    'endpoints': {
                        s.endpoint: {
                            ...
                            'billable_calls': s.successful_calls - s.cached_calls
                        }
                        for s in endpoint_list
                    },
            ...
            cost_info['summary'] = {
                ...
                'note': 'Refer to your API plans for specific pricing'
            }
    ...
    def log_statistics(self):
        """
        Log comprehensive statistics for all APIs and endpoints.
        ...
        logger.info("=" * 60)
        logger.info("API Monitoring Statistics")
        logger.info("=" * 60)
        ...
        for api_name, api_stats in stats['apis'].items():
            ...
            logger.info(f"  Success Rate: {100 - api_stats['error_rate']:.1f}%")
            logger.info(f"  Error Rate: {api_stats['error_rate']:.1f}%")
            logger.info(f"  Cache Hit Rate: {api_stats['cache_hit_rate']:.1f}%")
            logger.info(f"  Avg Response Time: {api_stats['average_response_time']:.2f}s")
            logger.info(f"  Total Wait Time: {api_stats['total_wait_time']:.2f}s")
            logger.info(f"  Endpoints: {len(api_stats['endpoints'])}")
            
            for endpoint, endpoint_stats in api_stats['endpoints'].items():
                logger.info(f"    - {endpoint}: {endpoint_stats['total_calls']} calls, "
                           f"{endpoint_stats['error_rate']:.1f}% errors")
    ...
    def export_statistics(self, filepath: Path):
        ...
        logger.info(f"Exported API statistics to {filepath}")
    ...
    def get_recent_errors(
        ...
        """
        Get recent errors for an API, endpoint, or all APIs.
        
        Args:
            api_name: Specific API to get errors for, or None for all APIs
        ...
        with self.lock:
            if api_name and endpoint:
                ...
                if key not in self.endpoint_stats:
                    return []
                return list(self.endpoint_stats[key].recent_errors)[-limit:]
            
            elif api_name:
                # Get errors for all endpoints of this API
                all_errors = []
    ...
    def get_metrics_by_time_range(
        ...
        Returns:
            List of APICallMetric objects within the time range
        ...
        Example:
            ...
            >>> metrics = monitor.get_metrics_by_time_range(last_hour, now, 'You.com')
            >>> print(f"You.com calls in last hour: {len(metrics)}")
        """
        with self.lock:
            filtered_metrics = [
...
def set_global_monitor(monitor: APIMonitor):
    """
    Set the global API monitor instance.
    
    Args:
        monitor: APIMonitor instance to use as global
    
    Example:
        >>> from tools.api_monitor import APIMonitor, set_global_monitor
        >>> custom_monitor = APIMonitor(failure_threshold=5)
